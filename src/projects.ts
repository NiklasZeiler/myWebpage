export const allProjects = [
    {
        name: 'Lieferino',
        description: $localize`Lieferando recreated with JavaScript`,
        img: 'lieferino.png',
        imgover: 'lieferino.png',
        type: 'JavaScript',
        url: 'https://niklas-zeiler.de/Lieferino/index.html'
    },
    {
        name: 'Pokedex',
        description: $localize`Pokedex created with an API and JavaScript`,
        img: 'pokedex.png',
        imgover: 'pokedex.png',
        type: 'JavaScript / RestAPI',
        url: 'https://niklas-zeiler.de/Pokedex/index.html'

    },
    {
        name: 'El Pollo Loco',
        description: $localize`Jump and run game written with JavaScript and object orientation`,
        img: 'el-pollo-loco.png',
        imgover: 'el-pollo-loco.png',
        type: 'JavaScript',
        url: 'https://niklas-zeiler.de/El-Pollo-Loco/index.html'
    },
    {
        name: 'My webpage',
        description: $localize`My own webpage`,
        img: 'meineWebseite.png',
        imgover: 'meineWebseite.png',
        type: 'Angular',
        url: 'https://niklas-zeiler.de'
    },
    {
        name: 'Ring of fire',
        description: $localize`Card game created with Angular and firebase`,
        img: 'ringoffire.png',
        imgover: 'ringoffire.png',
        type: 'Angular & Firebase',
        url: 'https://ring-of-fire-f4eb1.web.app/'
    },
    // {
    //     name: 'Simple-CRM',
    //     description: $localize`A Customer-Relationship-Management-System`,
    //     img: 'simple-crm.png',
    //     imgover: 'simple-crm.png',
    //     type: 'Angular & Firebase',
    //     url: 'https://simple-crm-96370.firebaseapp.com/'
    // }
];
